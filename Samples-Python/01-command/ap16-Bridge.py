...
def FunSerial(USBPort,USBRate,SendReceive,data_array,BridgeUSBPort, BridgeUSBRate):
  try:
    ...
    if   SendReceive == 'R':
       ...
       serBridge = serial.Serial(BridgeUSBPort, BridgeUSBRate, timeout=0.5)
       FunBridge(ser,serBridge)
    else:
...
def main(argv):
   ...
   try:
      ...
      opts, args = getopt.getopt(argv,"u:r:a:s:b:m:x",["ifile=","ofile=","action=","data_array=","bridg_port=","bridg_rate=","xxx="])
   ...
   for opt, arg in opts:
      if opt == '-h':
         ...
         data_array = arg
      elif opt in ("-b", "--bridg_port"):
         bridg_port = arg
      elif opt in ("-m", "--bridg_rate"):
         bridg_rate = arg
   ...
   print 'Bridge Source Port is ', bridg_port
   print 'Bridge Source Port rate is ', bridg_rate
   print 'xxx.py -u <USBPort> -u <USBRate> -a <R or T> -s<11,12,13>'
   FunSerial(USBPort,USBRate,RXTX,data_array,bridg_port,bridg_rate)   
