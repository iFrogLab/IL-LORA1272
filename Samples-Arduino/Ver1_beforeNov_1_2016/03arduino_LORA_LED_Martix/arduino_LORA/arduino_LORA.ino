...
void FunLora_5_write2(){ //byte iData[]){
...
void FunLora_5_write(String iStr){ //byte iData[]){
  ...
  char charBuf[len+2];
  iStr.toCharArray(charBuf, len+1);
  //Serial.println(iStr);
  //Serial.println(len);
  //Serial.println(charBuf);
    
  byte t1[] = {0xc1,0x05,len+1}; //,0x03,0x01,0x02,0x03};
  mySerial.write(t1,3);
 // mySerial.write(len,1 );
  mySerial.write(charBuf,len+1);
  //mySerial.write(t1,1);
  i=0;
  for(int j=0;j<999999;j++){
   if (mySerial.available()) {
...
void FunLora_6_read(){
  ...
  for(int j=0;j<99999;j++){
   if (mySerial.available()) {
    ...
    if(i==3){
      dataLen=t1;
    }else if(i>3 && i<=(3+dataLen) ){
      char bytes =t1;
      t2=t2+bytes;  //(String) strtol( t1, NULL, 16);
    }
     if(i>=(3+dataLen+1)){
       if(LastString!=t2){
          Serial.println(t2);
